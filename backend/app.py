...
# def initialize_predictor():
#     global predictor, players_index, pitcher_predictor, eligible_pitchers
...
#     last_pitch = pitch_season[pitch_season["yearID"] == LAST_YEAR].copy()
#     last_pitch = last_pitch[last_pitch["IP"] >= MIN_IP_STARTER]

#     ids = set(last_pitch["playerID"].astype(str).unique().tolist())
#     elig_people = people[people["playerID"].astype(str).isin(ids)][["nameFirst", "nameLast"]].dropna()

#     eligible_pitchers = sorted(
#         (elig_people["nameFirst"].str.strip() + " " + elig_people["nameLast"].str.strip()).unique().tolist()
#     )
...
#     # Build searchable hitter index from People.csv, filtered to eligible hitters
#     pi_df = people[["playerID", "nameFirst", "nameLast"]].dropna().copy()
#     pi_df["playerID"] = pi_df["playerID"].astype(str)
#     pi_df = pi_df[pi_df["playerID"].isin(eligible_ids)].copy()
...
#     print(f"Autocomplete eligible hitters (PA >= {MIN_PA_FULLTIME}): {len(players_index)}")
#     print(f"Autocomplete eligible starters (IP >= {MIN_IP_STARTER} in {LAST_YEAR}): {len(eligible_pitchers)}")
...
#     except Exception as e:
#         return jsonify({"error": f"Wins prediction failed: {str(e)}"}), 500
...
#     ids = set(last["playerID"].unique().tolist())
#     elig_people = people[people["playerID"].isin(ids)][["nameFirst","nameLast"]].dropna()
#     eligible_pitchers = sorted((elig_people["nameFirst"] + " " + elig_people["nameLast"]).unique().tolist())
...
#     if not q:
#         return jsonify({"results": eligible_pitchers[:50]})
#     out = [n for n in eligible_pitchers if q in n.lower()]
